            ...
            for tweet in tweets_all:
                try:
                    ...
                    if language_ in SUPPORTED_LANGUAGES.keys():
                        ...
                        tweets_list.append({'id': tweet['id'], 'sentiment': 'OtherLanguages', 'language':language_, 'sentiment_s':'Done'})
                except Exception as exp:
                    print(f'[update_sentiments]: [Exception] at Loading data! {exp}')
                threshold = min(4000, max_row)
                if len(sentiments_list) + len(tweets_list) >= threshold:
                ...
                if len(sentiments_list) + len(tweets_list) >= threshold:
                    ...
                    if solr.add_items_to_solr(core, tweets_list):
